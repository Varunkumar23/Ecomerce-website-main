...
export const placeOrder = async (req, res, next) => {
  try {
    ...
    // Debugging: Check the cart data before placing an order
    console.log("🛒 User Cart Before Order:", JSON.stringify(user.cart, null, 2));

    // Ensure cart items contain valid product references
    const orderProducts = user.cart
      .filter((item) => item.product && item.product._id) // Ensure product exists
      .map((item) => ({
        product: item.product._id, // Store only ObjectId
        quantity: item.quantity,
      }));

    if (orderProducts.length === 0) {
      return next(createError(400, "No valid products found in cart"));
    }

    console.log("✅ Final Order Products Array:", orderProducts);
...
export const getAllOrders = async (req, res, next) => {
  try {
    ...
    const orders = await Orders.find({ user: user.id }).populate('products.product');
